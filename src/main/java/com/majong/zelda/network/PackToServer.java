...
public class PackToServer {
    ...
    private void placebomb(Player player,boolean round) {
    	...
    	if(!HasShikaStone(player)) {
    		player.sendMessage(new TranslatableComponent("msg.anticheat.warn"),UUID.randomUUID());
    ...
    private void usemagnet(Player player) {
    	...
    	player.sendMessage(new TranslatableComponent("msg.zelda.magnetactivated"),UUID.randomUUID());
    	}else {
