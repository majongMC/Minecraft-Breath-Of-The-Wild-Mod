    ...
    public static void ancientdamage(LivingEntity living,@Nullable Entity attacker) {
    	if(!living.level.isClientSide) {
			...
			if(living.canChangeDimensions()) {
				if(attacker instanceof LivingEntity)
					living.hurt(living.level.damageSources().mobAttack((LivingEntity) attacker),20);
				else
					living.hurt(living.level.damageSources().generic(),20);
			}
			else {
				if(attacker instanceof LivingEntity)
					living.hurt(living.level.damageSources().mobAttack((LivingEntity) attacker),40);
				else
					living.hurt(living.level.damageSources().generic(),40);
			}
		}
	}
