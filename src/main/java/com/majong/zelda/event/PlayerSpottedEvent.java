...
public class PlayerSpottedEvent {
	...
	public static void onEntitySpotPlayer(LivingSetAttackTargetEvent event) {
		if(event.getTarget() instanceof PlayerEntity&&!event.getEntity().world.isRemote) {
			if(event.getEntity() instanceof GuardianEntity) {
				if(event.getEntity().world.getGameTime()%20==7) {
					if(event.getEntity() instanceof WalkingGuardianEntity) {
						Networking.SOUND.send(
		                    PacketDistributor.PLAYER.with(
		                            () -> (ServerPlayerEntity) event.getTarget()
		                    ),
		                    new SoundPack(2,new BlockPos(event.getEntity().getPosX(),event.getEntity().getPosY(),event.getEntity().getPosZ())));
					}
					else {
						Networking.SOUND.send(
			                    PacketDistributor.PLAYER.with(
			                    ...
			                    new SoundPack(3,new BlockPos(event.getEntity().getPosX(),event.getEntity().getPosY(),event.getEntity().getPosZ())));
					}
				}
			}
			if(event.getEntity() instanceof MollyBrinEntity||event.getEntity() instanceof PokBrinEntity) {
				if(event.getEntity().world.getGameTime()%20==8) {
						Networking.SOUND.send(
			                    ...
			                    new SoundPack(4,new BlockPos(event.getEntity().getPosX(),event.getEntity().getPosY(),event.getEntity().getPosZ())));
				}
			}
			if(Utils.ICE_AND_FIRE_LOADED&&event.getEntity() instanceof EntityCyclops) {
				...
				if(event.getEntity().world.getGameTime()%20==9) {
					Networking.SOUND.send(
				      PacketDistributor.PLAYER.with(
				                            () -> (ServerPlayerEntity) event.getTarget()
				             ),
				             new SoundPack(9,new BlockPos(event.getEntity().getPosX(),event.getEntity().getPosY(),event.getEntity().getPosZ())));
					}
				}
		}
	}
