...
public class ZeldaMessageGui extends Screen{
    ...
    public void render(PoseStack PoseStack, int mouseX, int mouseY, float partialTicks) {
		...
		for(int i=0;i<halfheart;i++) {
			...
			blit(PoseStack, x, y, 52, 0, 9, 9, 256, 256);
			drawCenteredString(PoseStack,this.font,new TranslatableComponent("x"+heal/2+""+(heal%2!=0?".5":"")),x+20,y,16777215);
			RenderSystem.setShaderTexture(0, ICONS);
		}
		x=(int) (0.52*w);
		y=(int) (0.5*h);
		...
		for(int i=0;i<halfhunger;i++) {
			...
			blit(PoseStack, x, y, 52, 27, 9, 9, 256, 256);
			drawCenteredString(PoseStack,this.font,new TranslatableComponent("x"+hunger/2+""+(hunger%2!=0?".5":"")),x+20,y,16777215);
		}
		switch(type) {
