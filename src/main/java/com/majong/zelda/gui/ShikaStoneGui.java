...
public class ShikaStoneGui extends Screen{
	...
	protected void init() {
		...
		this.magnet= Button.builder(Component.empty(), (button) -> {Networking.PACKWITHUUID.sendToServer(new PackToServer(5));this.onClose();}).bounds((int)(0.5*w-32), (int)(0.3*h), 32, 32).build();
		this.addRenderableWidget(magnet);
		this.Static= Button.builder(Component.empty(), (button) -> {Networking.PACKWITHUUID.sendToServer(new PackToServer(6));this.onClose();}).bounds((int)(0.5*w), (int)(0.3*h), 32, 32).build();
		this.addRenderableWidget(Static);
		this.ice= Button.builder(Component.empty(), (button) -> {Networking.PACKWITHUUID.sendToServer(new PackToServer(7));this.onClose();}).bounds((int)(0.5*w+32), (int)(0.3*h), 32, 32).build();
		this.addRenderableWidget(ice);
		this.camera= Button.builder(Component.empty(), (button) -> {Networking.PACKWITHUUID.sendToServer(new PackToServer(8));this.onClose();}).bounds((int)(0.5*w+64), (int)(0.3*h), 32, 32).build();
