...
public class ShikaStoneGui extends Screen{
	...
	protected void init() {
		...
		this.magnet= new Button((int)(0.5*w-32), (int)(0.3*h), 32, 32, new TranslationTextComponent("磁力"), (button) -> {Networking.PACKWITHUUID.sendToServer(new PackWithUUID(5));this.onClose();});
		this.addButton(magnet);
		this.Static= new Button((int)(0.5*w), (int)(0.3*h), 32, 32, new TranslationTextComponent("静止器"), (button) -> {Networking.PACKWITHUUID.sendToServer(new PackWithUUID(6));this.onClose();});
		this.addButton(Static);
		this.ice= new Button((int)(0.5*w+32), (int)(0.3*h), 32, 32, new TranslationTextComponent("制冰器"), (button) -> {Networking.PACKWITHUUID.sendToServer(new PackWithUUID(7));this.onClose();});
		this.addButton(ice);
		this.camera= new Button((int)(0.5*w+64), (int)(0.3*h), 32, 32, new TranslationTextComponent("照相机"), (button) -> {Networking.PACKWITHUUID.sendToServer(new PackWithUUID(8));this.onClose();});
